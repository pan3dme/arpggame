module fb {
    export class FubenRewardPanel extends UIConatiner {
        ...
        private t_lock_bg: UICompenent;
        private t_lock_lab: UICompenent;

        private moveAry: Array<UICompenent>;

        private passID: number = 1;
        private passObj: any;
        private loadConfigCom(): void {
            ...
            this.t_lock_bg.addEventListener(InteractiveEvent.Up, () => { }, this);
            this.t_lock_bg.addEventListener(InteractiveEvent.Down, () => { }, this);
            this.t_lock_lab = this._topRender.getComponent("t_lock_lab");

            var obj:any = TableData.getInstance().getData(TableData.tb_instance_stage_auto,1)
        ...
        private showRewardIconItem($rewardData: Array<item_reward_info>): void {
            var $len: number = Math.min($rewardData.length, 5);
            var xpos: number = (UIData.designWidth - ($len * 105 - 37)) / 2
            for (var i: number = 0; i < 5; i++) {
        ...
        public showBtnType($type: number): void {
            ...
            if ($type == 3) {
                var curPassID:number = GuidData.map.getPassID()
        ...
        public show($vo: s2c_send_instance_result): void {
            ...
            if ($vo.type == SharedDef.INSTANCE_SUB_TYPE_TRIAL
                || $vo.type == SharedDef.INSTANCE_SUB_TYPE_FACTION_TOWER) {//试炼塔
                btnType = 2;
                ...
                var obj: any = TableData.getInstance().getData(TableData.tb_instance_stage, GuidData.map.getPassID() + 1);
