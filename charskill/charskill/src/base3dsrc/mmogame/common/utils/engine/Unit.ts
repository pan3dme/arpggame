...
class BuffUnit {
    ...
    private changeBuffById($indx: number): void {
        ...
        var $effectid: number = this.unit.GetUInt16(SharedDef.UNIT_FIELD_BUFF_EFFECT_ID + id, di);
        if ($buffid != 0) {
    ...
    public getBuffByID(value: number): any {
        if (this.item && this.item[value]) {
...
class Unit extends GuidObject {
    ...
    public optimization:boolean = false;//当优化为true的时候 不显示

    public constructor(g: string = "") {
        super(g);

    }
    public ReadFromCopy(flags: number, bytes: ByteArray, evFilter: SyncEventFilter = null, applyNew: boolean = true): boolean {

        if (!this.buffUnit) {
            this.buffUnit = new BuffUnit(this)
        }
        return super.ReadFrom(flags, bytes, evFilter,applyNew);
    }
    ...
    public ReadFrom(flags: number, bytes: ByteArray, evFilter: SyncEventFilter = null, applyNew: boolean = true): boolean {
        if (this.ReadFromCopy(flags, bytes, evFilter,applyNew)) {
            ...
            if (flags & SyncEvent.OBJ_OPT_NEW) {
                ...
                var tempr: number = bytes.readFloat();
    ...
    public onCreated(): void {
        ...
        this.AddListen(SharedDef.UNIT_INT_FIELD_MOUNT_RIDE, ($binlog: BinLogStru) => {
            this.mountChange();
        })
        ...
        this.AddListen(SharedDef.UNIT_INT_FIELD_BOSS_OWN_FLAG, ($binlog: BinLogStru) => {
            this.sceneChar.refreshTittle();
        })
        ...
        this.AddListen(SharedDef.UNIT_FIELD_DAO, ($binlog: BinLogStru) => {
            this.sceneChar.showName();
        })

        this.AddListenString(SharedDef.UNIT_STRING_FIELD_DROP_OWNER_NAME, ($binlog: BinLogStru) => {
            this.sceneChar.refreshOwner();
        })
    ...
    public getFilevirtualCamp(): number {
    ...
    //称号
    public getUintFieldBossOwnFlag(): number {
    ...
    private getNeedCutPath(a: Vector2D, b: Vector2D): Array<number> {
        var $arr: Array<number> = new Array();
        var $n: number = Math.ceil(Vector2D.distance(a, b) / 100);
        var $lastPos: Vector2D = new Vector2D(a.x,a.y)
        for (var i: number = 1; i < $n; i++) {
            var $addV2d: Vector2D = new Vector2D(b.x - a.x, b.y - a.y)
