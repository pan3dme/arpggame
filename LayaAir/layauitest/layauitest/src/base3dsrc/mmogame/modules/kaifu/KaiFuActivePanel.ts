module kaifu {
    export class KaiFuActivePanel extends WindowUi {
        ...
        private _labAry: Array<FrameCompenent>;
        private initTabList(): void {
            if (this._labAry) {
                for (var i: number = 0; i < this._labAry.length; i++) {
        ...
        public playerDataChg(): void {
            var p: any = this.panelDic[this._currentIdx];
            p.refresh();
        ...
        public dayTargetList(saosrl: s2c_activity_opt_show_rank_list): void {
            ...
            // }
            for(var i:number=0;i<this.panelDic.length;i++){
                if(this.panelDic[i] instanceof KaifuDayTargetPanel){
                    (<KaifuDayTargetPanel>this.panelDic[i]).setListData(saosrl);
                    break;
                }
            }
        }
