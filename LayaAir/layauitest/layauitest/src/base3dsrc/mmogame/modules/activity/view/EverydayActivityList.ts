module activity {
    ...
    export class EverydayActivityList extends SList {
        ...
        public tickRefreshState(): void {
            if (ActivityModel.getInstance().compareAandB(this._aryList, this._type)) {
        ...
        public refreshDataByNewData(): void {
            //通过type，获得所对应的列表
            this._aryList = ActivityModel.getInstance().getList(this._type);
    ...
    export class EverydayActivityRender extends SListItem {
        ...
        private drawBtn($data: ActivityItemVo): void {
            ...
            if ($data.state == -1) {
                ...
                //活动今日未开启
                var imgUseRect1: UIRectangle = this.parentTarget.baseAtlas.getRec("Btn_6")
                ctx.drawImage(this.parentTarget.baseAtlas.useImg, imgUseRect1.pixelX, imgUseRect1.pixelY, imgUseRect1.pixelWitdh, imgUseRect1.pixelHeight, 0, 0, imgUseRect1.pixelWitdh, imgUseRect1.pixelHeight);
                ...
                //正在进行中
                if ($data.canreceive) {
                    var imgUseRect1: UIRectangle = this.parentTarget.baseAtlas.getRec("Btn_7")
                } else {
        ...
        public butClik(evt: InteractiveEvent): void {
            if (this.itdata && this.itdata.data) {
                ...
                if (vo.state == 1 || vo.state == 2) {
                    ...
                    if (vo.data.npc) {
                        var npcary: Array<number> = [3]
                        npcary = npcary.concat(vo.data.goto_sub);
                        //npcary = npcary.concat(vo.data.goto);
                        //console.log("---npcary--",npcary);
                        //npcary.push(2)
                        ModuleEventManager.dispatchEvent(new ActivityEvent(ActivityEvent.HIDE_ACTIVITY_EVENT))
