...
class GameMouseManager {
    ...
    public onSceneMouseDown($evt: InteractiveEvent): void {
        ...
        if (this.cantClikGround($mousePos) && GuidData.player) {
            ...
            if ($selectChar && !$selectChar.isDeath) {
                var $toDis: number = Vector2D.distance($selectChar.getAstarPos(), GameInstance.mainChar.getAstarPos())
    ...
    private walkNeeDToClik($selectChar: SceneChar, $mousePos: Vector2D): void {
        var $nrm: Vector3D = GameInstance.mainChar.getCurrentPos().subtract($selectChar.getCurrentPos());
        $nrm.normalize()
        $nrm.scaleBy(30);
        var item: Array<Vector2D> = AstarUtil.findPath3D(GameInstance.mainChar.getCurrentPos(), $selectChar.getCurrentPos().add($nrm));
        if (item) {
    ...
    private clikSceneChar($selectChar: SceneChar, $mousePos: Vector2D): void {
        if ($selectChar && !$selectChar.isDeath) {
    ...
    //点角色
    private mouseClikSceneChar($camVec: Vector3D, $hitPos: Vector3D): SceneChar {
        for (var i: number = 0; i < GameInstance.roleList.length; i++) {
            if (!(GameInstance.roleList[i] instanceof ScenePortal || GameInstance.roleList[i] instanceof SceneLoot)) {
                var $sceneChar: SceneChar = <SceneChar>GameInstance.roleList[i];
                var flag: boolean = $sceneChar.unit.isMain;
    ...
    //采集物品
    private mouseClikScenePortal($camVec: Vector3D, $hitPos: Vector3D): SceneChar {
        for (var i: number = 0; i < GameInstance.roleList.length; i++) {
            if (GameInstance.roleList[i] instanceof ScenePortal) {
                var $sceneChar: ScenePortal = <ScenePortal>GameInstance.roleList[i];
        ...
        if ($hitPos) {
            ...
            if ($v2d) {
               ...
               // var item: Array<Vector2D> = AstarUtil.findPath3D(GameInstance.mainChar.getCurrentPos(), $hitPos);
                var item: Array<Vector2D> =  AstarUtil.findPath2D(this.findMainCharNearCanWalkVe2(), AstarUtil.getGrapIndexByPos($hitPos))
                if (item) {
    ...
    private findMainCharNearCanWalkVe2(): Vector2D {
        var $baseV2d: Vector2D = GameInstance.mainChar.getAstarPos()
    ...
    public static moveQuestCell: boolean = false;  //不和任务移动冲突
