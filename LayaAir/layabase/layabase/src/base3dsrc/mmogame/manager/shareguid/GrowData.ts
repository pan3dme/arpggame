class GrowData extends GuidObject {
    ...
    public getWingModelByID($id: number): number {
        var targetData: any = TableData.getInstance().getData(TableData.tb_wings_bless, $id);
        var wingInfo: any = TableData.getInstance().getData(TableData.tb_wings_base, targetData.rank);
        return wingInfo.model;
    }
    //翅膀强化等级
     ...
     * 法宝装备列表
     */
    public gettalismanslotlist(): Array<TreasureWearVo> {
        var ary: Array<TreasureWearVo> = new Array
        for (let i = SharedDef.SPELL_TALISMAN_SLOT_START; i < SharedDef.SPELL_TALISMAN_SLOT_END; i = i + SharedDef.MAX_TALISMAN_SLOT_COUNT) {
            var vo: TreasureWearVo = new TreasureWearVo;
