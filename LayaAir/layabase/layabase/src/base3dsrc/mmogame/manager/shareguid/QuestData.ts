...
class QuestData extends GuidObject {
    ...
    public onBaseCreated(): void {
        ...
        this.AddListen(SharedDef.QUEST_FIELD_WELFARE_SEVEN_DAY_RECHARGE_PROCESS, ($binlog: any) => {
            ModuleEventManager.dispatchEvent(new welfare.WelfareEvent(welfare.WelfareEvent.CHG_REFILL));
        });
        this.AddListen(SharedDef.QUEST_FIELD_WELFARE_SEVEN_DAY_RECHARGE_EXTRA_FLAG, ($binlog: any) => {
            ModuleEventManager.dispatchEvent(new welfare.WelfareEvent(welfare.WelfareEvent.CHG_REFILL));
        });
    ...
    private refreshDailyQuestVo(): Array<QuestDataVo> {
        ...
        for (var i: number = 0; i < SharedDef.MAX_DAILY2_QUEST_COUNT; i++) {
