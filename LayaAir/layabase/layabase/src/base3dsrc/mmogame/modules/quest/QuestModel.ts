module quest {
    ...
    export class QuestTaskVo {
        public index: number;
        public meshBossVo: sboss.MeshBossVo
        public tb_quest: tb.TB_quest;
    ...
    export class QuestModel {
        ...
        public restNpcQuestTittle(): void {
            for (var i: number = 0; GuidData.quest&& GameInstance.roleList && i < GameInstance.roleList.length; i++) {
                var $tempChar: SceneChar = <SceneChar>GameInstance.roleList[i];
                if ($tempChar.unit && $tempChar.unit.isNpc()) {
                    var $showType: number=0
                    var $dk: Array<QuestTaskVo> = this.getItemShow();
                    for (var j: number = 0; j < $dk.length; j++) {
        ...
        public getItemShow(): Array<QuestTaskVo> {

           
            var taskItem: Array<QuestTaskVo> = new Array();
            taskItem.push(QuestModel.getInstance().getMainTaskVo())
            var $dailyQuestTaskVo: QuestTaskVo = QuestModel.getInstance().getDailyQuestVoTemp();
            if ($dailyQuestTaskVo) {
                //console.log("---$dailyQuestTaskVo---",$dailyQuestTaskVo);
                taskItem.push($dailyQuestTaskVo)
            }
            var $arr: Array<QuestTaskVo> = QuestModel.getInstance().getOtherTaskVo();
            for (var i: number = 0; i < $arr.length; i++) {
                if (quest.QuestModel.TrackDic[$arr[i].id] && $arr[i].tb_quest.ctype != 2) {
                    taskItem.push($arr[i])
                }
            }
            taskItem.sort(function (a: QuestTaskVo, b: QuestTaskVo): number {
        ...
        private getClikTaskItemByIndx($taskVo: quest.QuestTaskVo, indx: number): Array<number> {
            ...
            var s: number = 0;
            var e: number = 0;
            for (var i: number = 0; $taskVo.textrectH&&i < $taskVo.textrectH.length; i++) {
                    ...
                    case 7:
                        ...
                        if ($temp[1] != 0) {
                            if (GuidData.map.getMapID() == $temp[1]) {
                                ...
                                var $moveV2d: Vector2D = GuidData.map.tbMapVo.getTeleportmap($temp[1])
                                if ($moveV2d) {
                                    //console.log("走过去")
                                    ...
                                    MainCharControlModel.getInstance().setWalkPathFun(AstarUtil.findPath2D(GameInstance.mainChar.getAstarPos(), $moveV2d), () => { GameMouseManager.getInstance().walkPathComplete() });
        ...
        private openPanel(value: number, $data: any): void {
            ...
            var $obj: any = TableData.getInstance().getData(TableData.tb_system_base, Number(value * 10 + page))
            if($obj){
