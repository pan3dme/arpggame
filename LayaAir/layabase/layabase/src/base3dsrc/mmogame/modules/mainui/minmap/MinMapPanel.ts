module minmap {
    export class MinMapPanel extends UIConatiner {
        ...
        private loadConfigCom(): void {
            ...
            this.addChild(<UICompenent>this._bottomRender.getComponent("a_email_bg"));

            //this.addChild(this._topRender.getComponent("a_open_map"));
            this.a_email_icon = this.addChild(<UICompenent>this._topRender.getComponent("a_email_icon"));//this.addEvntBut("a_email_icon", this._topRender)
            this.a_email_icon.addEventListener(InteractiveEvent.Up,this.emailClick,this);

            var red: RedPointCompenent = this._redPointRender.getRedPointUI(this, 3, new Vector2D(this.a_email_icon.x + this.a_email_icon.width + 3, this.a_email_icon.y - 4));
        ...
        public emailClick(evt: InteractiveEvent): void {
            ModulePageManager.openPanel(SharedDef.MODULE_MAIL);
        }
        private posStr:string;
