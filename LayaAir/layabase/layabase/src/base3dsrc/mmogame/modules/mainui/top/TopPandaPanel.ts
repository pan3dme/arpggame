module topui {
    ...
    export class ActivityManager {
        ...
        */
        private nodeDic: any = { 504: 74, 601: 112, 111: 116, 501: 120 };
        ...
        public getUiById($id: number): TopIconVo {
            for (var i = 0; i < this._itemUi.length; i++) {
                var tb: tb.TB_system_icon = this._itemUi[i].ui.data
                if (tb.id == $id) {
    ...
    export class TopPandaPanel extends UIVirtualContainer {
        ...
        public getRefillPos() {
            if(this._activityManager && this._activityManager.visible){
                var topIconVo:TopIconVo = this._activityManager.getUiById(SharedDef.MODULE_FIRST_RECHARGE);
                if(topIconVo){
                    var eee:refill.RefillEvent = new refill.RefillEvent(refill.RefillEvent.POP_Refill_EVENT);
                    eee.data = topIconVo
