        ...
        private btnClick(evt: InteractiveEvent): void {
            ...
            if (this.itdata && this.itdata.data) {
                ...
                if (vo.state == 2) {
                    ...
                    //补领事件
                    var $getbackobj: any = TableData.getInstance().getData(TableData.tb_welfare_checkin, GuidData.player.getBackTimes() + 1)
                    if ($getbackobj) {
                        AlertUtil.show("补签需要消耗" + getResName($getbackobj["getback_cost"][0][0]) + $getbackobj["getback_cost"][0][1] + "个，是否补签?", "补签提示", (a: any) => {
                            if (a == 1) {
                                costRes($getbackobj["getback_cost"][0], () => {
