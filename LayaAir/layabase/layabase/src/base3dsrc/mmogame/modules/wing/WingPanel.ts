module wing {
    ...
    export class WingPanel extends WindowUi {
        ...
        public applyLoad(): void {
            this._baseUiAtlas.setInfo("ui/uidata/wing/wing.xml", "ui/uidata/wing/wing.png", () => { this.loadConfigCom() }, "ui/uidata/wing/wingpc.png");
        }
        ...
        public jinjiePanel: WingJinjiePanel;
            ...
            if (this._canclick) {
                ...
                if (this._currentIdx == 0) {
                    if(this.jinjiePanel.autoplay){
                        AlertUtil.show("当前正在自动升阶神羽，是否取消？", "提示", (a: any) => {
                            if (a == 1) {
                                this.jinjiePanel.removeTick();
                            }
                        }, 2, ["是", "否"])
                        return;
                    }
        ...
        private drawBaseWingInfo(): void {
            ...
            var wingInfo: any = TableData.getInstance().getData(TableData.tb_wings_base, targetData.rank);
            var nameStr:string = getChiNum(wingInfo.id) + "阶  " + wingInfo.name
            if(!this.disableqh){
    ...
    export class WingJinjiePanel extends UIConatiner {
         ...
         */
        public drawBtn() {
            var $rec: UIRectangle = this.b_allbtn.uiRender.uiAtlas.getRec(this.b_allbtn.skinName);
            var ctx: CanvasRenderingContext2D = UIManager.getInstance().getContext2D($rec.pixelWitdh, $rec.pixelHeight, false);
            ...
            var imgUseRect1: UIRectangle = this.b_allbtn.uiRender.uiAtlas.getRec(usemsg);
        ...
        private getsmeltallstate(): boolean {
            ...
            var $smeltobj: any = TableData.getInstance().getData(TableData.tb_vip_uplev, 1);
