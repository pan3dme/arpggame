module skillUi {
    ...
    export class SkillPanel extends WindowUi {
        ...
        public getData(): void {
            ...
            for (var i: number = 0; i < lockObj.realmbreak_unlocks.length; i++) {
                var daoObj:any = TableData.getInstance().getData(TableData.tb_realmbreak_base, lockObj.realmbreak_unlocks[i][0]);
        ...
        public drawSelSkill($data: SkillItemData, redPoint: boolean): void {
            ...
            if ($data.lev == 0) {
                ...
                var lockObj:any = this.lockDic[$data.id];
                var lockStr:string;
                if(lockObj.lev > 0){
    ...
    export class SkillItemIcon {
        ...
        private drawLev(): void {
            if (this.data.lev > 0) {
                ...
                var lockObj:any = this.panel.lockDic[this.data.id];
                var lockStr:string;
                if(lockObj.lev > 0){
