module strengthgem {
    ...
    export class StrengTab2 extends UIVirtualContainer {
        ...
        private drawEqu($ui: UICompenent, $select: boolean, $type: string) {
            ...
            if (equData) {
                StrengUtil.setEquIcon($ui, geteqiconIconUrl(equData.entryData.icon), stateary, $select, $type, equData.entryData.quality, equData.entryData.realmbreak_level);
            } else {
        ...
        private drawBtn($data: GemVo, $index: number) {
            ...
            if ($data.state == 1) {
                ...
                // for (var i = 0; i < tab_gem_part.gem_array.length; i++) {
                    if (tab_gem_part.unlock_refine_lv[$index] > NewStrengModel.getInstance().getrefiningvo($data.partid).partlevstar[0]) {
                        LabelTextFont.writeSingleLabel(this._baseRender.uiAtlas, this.UnlockUIAry[$index].skinName, StrengUtil.equProp[$data.partid - 1] + "部位精炼达到" + (tab_gem_part.unlock_refine_lv[$index] + 1) + "段解锁", 14, TextAlign.RIGHT, ColorType.Brown7a2f21);
                        return;
                    }
                    if (tab_gem_part.unlock_strength_lv[$index] > NewStrengModel.getInstance().getstrengvo($data.partid).partlev) {
                        LabelTextFont.writeSingleLabel(this._baseRender.uiAtlas, this.UnlockUIAry[$index].skinName, StrengUtil.equProp[$data.partid - 1] + "部位强化+" + tab_gem_part.unlock_strength_lv[$index] + "解锁", 14, TextAlign.RIGHT, ColorType.Brown7a2f21);
                        return;
                    }
                // }
