module msgtip {
    export class MsgZhanliInfoPanel extends UIConatiner {
        ...
        private loadConfigCom(): void {
            ...
            this.b_info_bg = this.addChild(<UICompenent>this._bottomRender.getComponent("b_info_bg"));
        ...
        private cellNameItem: Array<FrameCompenent>
        private cellTextItem: Array<FrameCompenent>

        private getBaseData(): Array<Vector2D>
        {
            var $arr:Array<Vector2D>=new Array
            var $len: number = random(12);
        ...
        public setData($arr: Array<Vector2D>): void {
          //  $arr = this.getBaseData()
            ...
            for (var i: number = 0; i < $arr.length&&i<12; i++) {
                var b_cell_name: FrameCompenent = <FrameCompenent>this.addChild(this._midRender.getComponent("b_cell_name"));
                b_cell_name.y += i * 25;
                b_cell_name.goToAndStop(i);
                var $frameid: number = $arr[i].x - 1;
                this.cellNameItem.push(b_cell_name)
                this.drawCellNameToStr(b_cell_name, ColorType.Yellowffe9b4+ getKeyProById($arr[i].x));

                var b_cell_txt: FrameCompenent = <FrameCompenent>this.addChild(this._midRender.getComponent("b_cell_txt"));
                b_cell_txt.y += i * 25;
        ...
        private drawCellNameToStr($ui: FrameCompenent, $str: string): void {
        ...
        private b_info_bg:UICompenent
        private drawTextToName($ui: FrameCompenent, $num: number,$id:number): void {
  
            var $toRect: Rectangle = $ui.getSkinCtxRect()
            var $ctx: CanvasRenderingContext2D = UIManager.getInstance().getContext2D($toRect.width, $toRect.height, false);

            var $color: string = $num > 0?ArtFont.num7: ArtFont.num6;
            var $txt: string
            if ($id < 12) {
