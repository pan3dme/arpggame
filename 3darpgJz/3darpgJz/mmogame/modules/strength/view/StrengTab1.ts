module strengthgem {
    ...
    export class StrengTab1 extends UIVirtualContainer {
        ...
        private drawEqu($ui: UICompenent, $select: boolean, $type: string) {
            ...
            if (equData) {
                StrengUtil.setEquIcon($ui, geteqiconIconUrl(equData.entryData.icon), aa.partlevstar, $select, $type, equData.entryData.quality,equData.entryData.level);
            } else {
                StrengUtil.setEquNoIcon($ui, aa.partid, aa.partlevstar, $select, $type);
            }
        }
