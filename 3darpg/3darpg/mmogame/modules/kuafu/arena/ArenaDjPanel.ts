module kuafu {
    ...
    export class ArenaDjPanel extends UIConatiner {
        ...
        private _itemDjRoleAry: Array<ItemDjRole>;

        private _myRank: UICompenent;
        private _myForce: UICompenent;
        private _myDh: UICompenent;
        private _timeLab: UICompenent;
        private challengeLab: UICompenent;
        private c_vip_add: UICompenent;
        ...
        private initUI(): void {
            ...
            for (var i: number = 0; i < 3; i++) {
                var item: ItemDjRole = new ItemDjRole;
        ...
        private clearCD($e: InteractiveEvent): void {
            UiTweenScale.getInstance().changeButSize($e.target);
            var $addTime: number = GameInstance.getGameSecond(GuidData.instanceData.getDJcdtime());
            if ($addTime <= 0) {
                msgtip.MsgTipManager.outStr(ColorType.colorce0a00 + "无冷却时间，不需要清除", 99);
                return;
            }
            var mins: number = Math.ceil($addTime / 60);
            AlertUtil.show(ColorType.Brown7a2f21 + "您是否花费" + ColorType.Green2ca937 + mins + "元宝" + ColorType.Brown7a2f21 + "重置挑战冷却时间(1分钟=1元宝)", "",
                (val: number) => {
                    if (val == 1) {
                        costRes([1,mins], () => {
        ...
        private drawTime(): void {
            ...
            if ($addTime > 0) {
                ...
                var timeStr: string = TimeUtil.getLocalTime3($addTime);
        ...
        private drawChallengNum(): void {
            var aaa: number = GuidData.instanceData.getHasChallengeNum();
            var obj: any = TableData.getInstance().getData(TableData.tb_doujiantai_base, 1);
            var bbb: number = obj.dailytimes;
            var $kk: string = (bbb - aaa + GuidData.instanceData.getCanbuyChallengeNum()) + "/" + bbb;
        ...
        public setListData($ary: Array<KuaFu1v1BangData>): void {
            for (var i: number = 0; i < this._itemDjRoleAry.length; i++) {
                this._itemDjRoleAry[i].setData($ary[i]);
            }
        }

        private refreshEnemy($e: InteractiveEvent): void {
            UiTweenScale.getInstance().changeButSize($e.target);
        ...
        private showBuyMeshBoss($e: InteractiveEvent): void {
            UiTweenScale.getInstance().changeButSize($e.target);
            var hasNum: number = GuidData.instanceData.getHasChallengeNum();
            var tabObj: any = TableData.getInstance().getData(TableData.tb_doujiantai_base, 1)
            var maxNum: number = tabObj.dailyTimes + tb.TB_vip_base.get_TB_vip_baseById(GuidData.player.getVipLevel()).djtBuyTimes;
            var curBuyNum: number = GuidData.instanceData.getCanbuyChallengeNum();
            if (curBuyNum >= hasNum) {
            ...
            var tbSize: number = tabObj.buyInfo.length;

            //var canBuyNum:number = hasNum - curBuyNum;

            for (var i: number = curBuyNum; i < tbSize && i < hasNum; i++) {
        ...
        private _rewardPanel: ArenaDjRewardPanel;
        public itemIconClick($e: InteractiveEvent): void {
            ...
            if ($e.target == this._logBtn) {
                ...
                this._logPanel.load(() => {
                    this._logPanel.show(this._baseUiAtlas, 1);
                })

            } else if ($e.target == this._rewardBtn) {
                if (!this._rewardPanel) {
                    this._rewardPanel = new ArenaDjRewardPanel();
                }
    ...
    export class ItemDjRole {
        public role: Person2DChar;
        public rankUI: UICompenent
        public nameUI: UICompenent;
        public froceUI: UICompenent
        public challengeBut: UICompenent;
        ...
        public setData($data: KuaFu1v1BangData): void {
            this.data = $data;
