class BagData extends GuidObject {
    ...
    public static uipos: Vector2D = new Vector2D();
    ...
    public useItem($id:number): void {
        var uid: string = this.getPlayerGuid() + ";" + $id;
        NetManager.getInstance().protocolos.bag_item_user(uid, 1);
    }

    public getExpItemList(): Array<BagItemData> {
        var ary: Array<BagItemData> = new Array;
        var list: Array<BagItemData> = this.getGemBgData();
        var size:number = GuidData.bag.getBagSize(BagData.TYPE_GEM);
    ...
    public getOutLineItemList(): Array<BagItemData> {
        var ary: Array<BagItemData> = new Array;
        var list: Array<BagItemData> = this.getGemBgData();
        var size:number = GuidData.bag.getBagSize(BagData.TYPE_GEM);
        for (var i: number = 0; i < size; i++) {
