module faction {
    ...
    export class FactionEvent extends BaseEvent {
        ...
        public static SHOW_FACTION_HONOR_EVENT: string = "show_faction_honor_event";
    ...
    export class FactionProcessor extends BaseProcessor {
        ...
        protected receivedModuleEvent($event: BaseEvent): void {
            ...
            if ($event instanceof FactionLeaguEvent) {
                if ($event.type == FactionLeaguEvent.REFRESH_SESSION_EVENT) {
        ...
        private showActivePanel($data: any): void {
            if (!GuidData.faction) {
            ...
            this.activePanel.load(() => {
                ...
                this.activePanel.show($data);
            })
        }

        private alertpanel() {
            AlertUtil.show("模块未开启,是否前往主殿提升等级？", "提示", (a: any) => {
                if (a == 1) {
                    //购买vip
        ...
        private showFactionUi($tab: number): void {
            ...
            $obj.load(() => {
                ...
                if (!$tab) {
                    if ($obj instanceof ApplyFactionUiPanel) {
                        $tab = SharedDef.MODULE_FACTION_RECRUIT
                    } else if ($obj instanceof ExistFactionUiPanel) {
        ...
        private _honorPanel: FactionHonorPanel
        private showHonorpanel() {
            if (!this._honorPanel) {
                this._honorPanel = new FactionHonorPanel();
            }

            this._honorPanel.load(() => {
                this._honorPanel.show();
            })
        }
        ...
        protected listenModuleEvents(): Array<BaseEvent> {
            return [
                ...
                new FactionLeaguEvent(FactionLeaguEvent.REFRESH_SESSION_EVENT),
