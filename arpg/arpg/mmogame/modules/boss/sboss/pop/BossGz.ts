module sboss {
    ...
    export class BossGz extends WindowCentenMin {
        ...
        public show($url:string = null): void {
        ...
        public resetData(): void {
            LoadManager.getInstance().load(Scene_data.fileRoot + "txt/quanminboss.txt", LoadManager.XML_TYPE, ($str: string) => {
                $str = $str.replace(/\r/g, "");
                //notepad++默认一个回车键入\n\r ,所以默认将所有的\r隐藏
        ...
        public setUrlData($url:string):void{
            LoadManager.getInstance().load(Scene_data.fileRoot + "txt/" + $url + ".txt", LoadManager.XML_TYPE, ($str: string) => {
                $str = $str.replace(/\r/g, "");
                //notepad++默认一个回车键入\n\r ,所以默认将所有的\r隐藏
                var aaa = LabelTextFont.writeTextLabel(this._baseRender.uiAtlas, this.b_gonggao.skinName, $str, 16, TextAlign.LEFT, 330, ColorType.color9a683f);
                this._txtHight = aaa[1];
