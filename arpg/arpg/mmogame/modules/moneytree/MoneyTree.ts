module moneytree {
    ...
    export class MoneyTree extends WindowCentenMin {
        ...
        public drawReward() {
            ...
            var $levobj: any = TableData.getInstance().getData(TableData.tb_moneytree_lv, GuidData.player.getLevel());
            for (var i = 0; i < bbb.length; i++) {
                if (i < this._rewardAry.length) {
                    this._rewardAry[i].data = bbb[i];
                    var reward: Array<number> = $levobj["gift_3"];
        ...
        public rewardClik(evt: InteractiveEvent): void {
            ...
            if ($data) {
                if ($data.state == 1) {
                    ...
                    var obj: tb.TB_item_template = tb.TB_item_template.get_TB_item_template(reward[0]);
