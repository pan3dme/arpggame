class NetBaseProcessor extends BaseProcessor {
    ...
    private smsgRankListQueryResult($byte: ByteArray): void {
        ...
        switch (rankTypeId) {
            ...
            case SharedDef.RANK_TYPE_DOUJIANTAI:
                ...
                break;
            case SharedDef.RANK_TYPE_3V3:
                var kuafuevt: kuafu.KuaFuEvent = new kuafu.KuaFuEvent(kuafu.KuaFuEvent.KUAFU_ARENA_3V3_RANK_EVENT);
                kuafuevt.data = { list: $list, self: $selfLevel, all: allNum };
                ModuleEventManager.dispatchEvent(kuafuevt);
                break;
            default:
                break
    ...
    public smsgExpUpData($byte: ByteArray): void {
        ...
        var $add: number = $byte.readUint8();
        // var $vipExp: number = $byte.readInt32();
        // var $color: string = "[ff00ff]"
        var $expStr: string = ($exp > 0 ? "+" : "") + Snum($exp);
        if ($exp > 0) {
            $expStr += "[" + $add + "%]"
            ...
            $textJumpUiVo.endtime = $textJumpUiVo.starttime + 1000;
            //飘字初始化位置均为玩家头顶
            ...
            // //console.log("经验加了");
            BloodManager.getInstance().setExpJumpNum($textJumpUiVo);
            // console.log("----$exp---", $textJumpUiVo.starttime);
        }
    ...
    public bookResult($byte: ByteArray): void {
        var $vo: s2c_booking_money_result = new s2c_booking_money_result;
        s2c_booking_money_result.read($vo, $byte);
        if ($vo.result) {
            GamePay.applyPay($vo.orderid);
        }
    }
    public bookResult481($byte: ByteArray): void {
        console.log("bookResult481==>")
        var $vo: s2c_lottery_recharge_result = new s2c_lottery_recharge_result;
        s2c_lottery_recharge_result.read($vo, $byte);
        console.log($vo.indx)


    }
    public msgpingpong($byte: ByteArray): void {
        GameInstance.pingpontm = TimeUtil.getTimer();
        //console.log(GameInstance.pingpontm)
  
    }
    public initPingPongTm(): void {
        TimeUtil.addTimeTick(10000, () => {
            if ((TimeUtil.getTimer() - GameInstance.pingpontm) > (2000*60)) {
                AlertUtil.show("你已经断开网络","提示", (flag) => {
                    if (flag == 1) {
    ...
    public getHanderMap(): Object {
       ...
       // this.initPingPongTm()
        ...
        obj[Protocols.SMSG_BOOKING_MONEY_RESULT] = ($byte: ByteArray) => { this.bookResult($byte) };
        obj[Protocols.SMSG_LOTTERY_RECHARGE_RESULT] = ($byte: ByteArray) => { this.bookResult481($byte) };
        obj[Protocols.MSG_PING_PONG] = ($byte: ByteArray) => { this.msgpingpong($byte) };
        return obj;
    }
