module newbieguide {
    ...
    export class NewbieguideProcessor extends BaseProcessor {
        ...
        private levChg(): void {
            var ary: Array<BagItemData> = GuidData.bag.getEquBgData()
            for (var i: number = 0; i < ary.length; i++) {
                var $EquMeshVo:BagItemData = ary[i]
                if(!$EquMeshVo){
                    continue;
                }
                var bagitem: BagItemData = GuidData.bag.getEquByPart($EquMeshVo.entryData.pos);
                var disableEqu:boolean = !this.testGender($EquMeshVo.entryData);
                if(disableEqu){
                    continue;
                }
