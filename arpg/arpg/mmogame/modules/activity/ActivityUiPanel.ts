module activity {
    ...
    export class ActivityUiPanel extends WindowUi {
        ...
        private loadConfigCom(): void {
            ...
            for (var i = 0; i < 2; i++) {
                var b = <SelectButton>this.addChild(this._bgRender.getComponent("tab" + i));
                // a.data = SharedDef.MODULE_MIX_STRENGTH + i;
                b.data = ActivityType.DAILY + i;
            ...
            this.addChild(<UICompenent>this._baseRender.getComponent("b_line"));
        ...
        private showTabPage($type: number) {
            if (!this.everydayActivityList) {
        ...
        public setProgressAndAchievement($num: number): void {
        ...
        private _KeyLocation = [225, 361, 495, 630, 765];
        ...
        public refreshTime() {
            var str: string = "离线挂机时间剩余: " + Math.floor(GuidData.player.getHangUpTime() / 60) + "时" + GuidData.player.getHangUpTime() % 60 + "分"
        ...
        public show(): void {
            ...
            this.selectedTab(ActivityType.DAILY);
            this.resetData();
            this.resize();
        }
        public hide(): void {
        ...
        public butClik(evt: InteractiveEvent): void {
            switch (evt.target) {
                case this.w_close:
                    ...
                    break
                case this.b_btn:
                    var itemlist: Array<BagItemData> = GuidData.bag.getOutLineItemList()
                    if (itemlist.length > 0) {
                        ...
                        AlertUtil.show(ColorType.Brown7a2f21 + "是否跳转至商城购买离线卡？", "",
                            (val: number) => {
                                if (val == 1) {
