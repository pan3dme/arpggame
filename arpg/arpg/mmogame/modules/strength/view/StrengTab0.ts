    ...
    export class StrengTab0 extends UIVirtualContainer {
        ...
        private initView(): void {
            ...
            for (var i = 0; i < 10; i++) {
                ...
                this.StarUIAry.push(<FrameCompenent>this.addChild(this._topRender.getComponent("b_star" + i)));
            }
        ...
        private drawEqu($ui: UICompenent, $select: boolean, $type: string) {
            ...
            if (equData) {
                StrengUtil.setEquIcon($ui, geteqiconIconUrl(equData.entryData.icon), [aa.curtab.rank, aa.curtab.star], $select, $type, equData.entryData.quality, equData.entryData.realmbreak_level);
            } else {
                StrengUtil.setEquNoIcon($ui, aa.partid, [aa.curtab.rank, aa.curtab.star], $select, $type);
            }
        }
        ...
        public resetData($data: StrengVo) {
            ...
            for (var i = 0; i < 10; i++) {
                if ($data.curtab.star > i) {
                    (<FrameCompenent>this.StarUIAry[i]).goToAndStop(0);
                } else {
                    (<FrameCompenent>this.StarUIAry[i]).goToAndStop(1);
                }
            }
