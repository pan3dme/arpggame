class SceneCharManager {
    ...
    public constructor(){
        ...
        var maxID:number = TableData.getInstance().getTabMaxID(TableData.tb_char_info)
        for(var i:number = 1;i<=maxID;i++){
            var obj:any = TableData.getInstance().getData(TableData.tb_char_info,i);
            this.roBootDic[obj.dummy_entry] = true;
        }
    }

    private roBootDic:any;
    private testRoBoot($entry): boolean {
