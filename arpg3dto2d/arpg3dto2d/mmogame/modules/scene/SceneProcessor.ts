...
class SceneProcessor extends BaseProcessor {
    ...
    public showLoot(time: number, obj: any, targetPos: Vector2D,idx:number): void {

        TimeUtil.addTimeOut(idx * 100, () => {
            GameInstance.addSceneChar(obj);
            var targetPy:number = obj.py + 80;
            var pos: Vector3D = AstarUtil.getWorldPosByStart2D(targetPos);
            pos.y = AstarUtil.getHeightByPos(pos);
            TweenLite.to(obj,0.3,{py:targetPy,ease:Linear.easeNone,onComplete:()=>{
    ...
    public smsgFullizehp($byte: ByteArray): void {
        var $str: string = $byte.readString();
        console.log($str)
    ...
    public getHanderMap(): Object {
        ...
        obj[Protocols.SMSG_FULLIZE_HP] = ($byte: ByteArray) => { this.smsgFullizehp($byte) };
