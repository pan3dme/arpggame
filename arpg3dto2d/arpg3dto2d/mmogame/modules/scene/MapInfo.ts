class MapInfo extends GuidObject {
    ...
    public onBaseCreated(): void {
        ...
        this.AddListen(SharedDef.MAP_INT_FIELD_WALL, ($binlog: any) => { this.mapBlock() });
    ...
    public getExpFubenInfo(): any {
        //return this.GetUInt32(SharedDef.GROUP_EXP_INSTANCE_FIELD_CNT);
        var obj: any = new Object;
        var num: number = this.GetUInt32(SharedDef.GROUP_EXP_INSTANCE_FIELD_CNT);
        obj.num = num;
        var flag: number = -1;
        var hasData: boolean = false;
        var pguid: string = GuidData.player.getGuid();
        for (var i: number = SharedDef.GROUP_EXP_INSTANCE_STR_FIELD_INFO_START; i < SharedDef.GROUP_EXP_INSTANCE_STR_FIELD_INFO_END; i += SharedDef.MAX_EXP_INFO_STR_COUNT) {
            flag++;
            var s1: string = this.GetStr(i + SharedDef.GROUP_EXP_INSTANCE_STR_GUID);
        ...
        if (hasData) {
            var start: number = SharedDef.GROUP_EXP_INSTANCE_INT_FIELD_INFO_START + SharedDef.MAX_EXP_INFO_INT_COUNT * flag
            var exp: number = this.GetDouble(start + SharedDef.GROUP_EXP_INSTANCE_INT_EXP);
            obj.exp = exp;
            var eff: number = this.GetUInt32(start + SharedDef.GROUP_EXP_INSTANCE_EFFECT);
            obj.eff = eff;
            var buy: number = this.GetUInt32(start + SharedDef.GROUP_EXP_INSTANCE_INT_MONEY_BUY);
            obj.buy = buy;
        } else {
            obj.exp = 0;
            obj.eff = 0;
    ...
    public getFactionFubenInfo(): any {
        var obj: any = new Object;

        var score1: number = this.GetUInt32(SharedDef.FACTION_MATCH_INSTANCE_INT_FIELD_SCORE_1);
        var member1: number = this.GetUInt32(SharedDef.FACTION_MATCH_INSTANCE_INT_FIELD_MEMBER_COUNT_1);
        var flagNum1: number = 0;
        for (var i: number = 0; i < 8; i++) {
            if (this.GetBit(SharedDef.FACTION_MATCH_INSTANCE_INT_FIELD_FLAG_1, i)) {
                flagNum1++;
            }
        }
        var icon1: number = this.GetUInt32(SharedDef.FACTION_MATCH_INSTANCE_INT_FIELD_ICON_1);

        var score2: number = this.GetUInt32(SharedDef.FACTION_MATCH_INSTANCE_INT_FIELD_SCORE_2);
        var member2: number = this.GetUInt32(SharedDef.FACTION_MATCH_INSTANCE_INT_FIELD_MEMBER_COUNT_2);
        var flagNum2: number = 0;
        for (var i: number = 0; i < 8; i++) {
            if (this.GetBit(SharedDef.FACTION_MATCH_INSTANCE_INT_FIELD_FLAG_2, i)) {
                flagNum2++;
            }
        }
        var icon2: number = this.GetUInt32(SharedDef.FACTION_MATCH_INSTANCE_INT_FIELD_ICON_2);

        var g1: string = this.GetStr(SharedDef.FACTION_MATCH_INSTANCE_STRING_FIELD_FACTION_GUID_1);
        var g2: string = this.GetStr(SharedDef.FACTION_MATCH_INSTANCE_STRING_FIELD_FACTION_GUID_2);
    ...
    public getMyFactionFlag(): number {
