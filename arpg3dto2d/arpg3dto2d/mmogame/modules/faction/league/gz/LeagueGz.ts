module faction {
    ...
    export class LeagueGz extends WindowMinUi {
        ...
        private posx: number = 208;
        private posy: number = 97;
        private posW: number = 512;
        private posH: number = 400;
        ...
        public resetData(): void {
            LoadManager.getInstance().load(Scene_data.fileRoot + "txt/league_gz.txt", LoadManager.XML_TYPE, ($str: string) => {
                $str = $str.replace(/\r/g, "");
                //notepad++默认一个回车键入\n\r ,所以默认将所有的\r隐藏
                var aaa = LabelTextFont.writeTextLabel(this._baseRender.uiAtlas, this.b_gonggao.skinName, $str, 16, TextAlign.LEFT, 500, ColorType.color9a683f);
