    ...
    private playSkillEffect(skillid: number): void {
        ...
        if ($skilldata.tb_skill_base.effect_file && $skilldata.tb_skill_base.effect && $skilldata.tb_skill_base.is_remain == 0) {
            ...
            if ($skill) {
                ...
                if ($skilldata.tb_skill_base.isOrbit == 1888888) { //锁定技能
                    if (GameInstance.attackTarget) {
...
class AotuSkillManager {
    ...
    public update(t): void {
        ...
        if (this._aotuBattle && GameInstance.attackTarget && GameInstance.attackTarget && GameInstance.mainChar.curentAction == CharAction.WALK) {//如果正在行走，并离攻击对象距离很近时设定为
        ...
        if (this._aotuBattle && this.nextPlaySkillTm < TimeUtil.getTimer()) {
            ...
            var uvScale: number = $unit.getHp() / $unit.getMaxHp();
            if (uvScale < 0.75) {
                MainCharControlModel.getInstance().clikEat()
            }
            if (!GameInstance.attackTarget || !GameInstance.attackTarget.visible) {//没有攻击对象.找一个最近的攻击对象
    ...
    private getNextPlaySkillVo(): tb.SkillDataVo {
    ...
    private isCanPlaySkillDis($selectSkill: tb.SkillDataVo): boolean {
    ...
    private needAttackToSelect(): void  //有攻击对象，将对其进行攻击
    {
        ...
        if ($curentAction == CharAction.STANAD || $curentAction == CharAction.ATTACK_010 || $curentAction == CharAction.ATTACK_020) {
            ...
            var $selectSkill: tb.SkillDataVo = this.getNextPlaySkillVo();
            if (this.isCanPlaySkillDis($selectSkill)) {//在攻击范围内
                ...
                if (GameInstance.attackTarget && GameInstance.attackTarget.onStage) {
                    ...
                    this.aotuMoveMapPath(() => { this.walkPathComplete() })
                }
    ...
    private aotuMoveMapPath($fun: Function = null): void {
        if (GuidData.map.tbMapVo.path && GuidData.map.tbMapVo.path.length) {
            var $self2D: Vector2D = GameInstance.mainChar.getAstarPos()
            var $dis: number = 10000;
            var nearId: number;
            var $len: number = GuidData.map.tbMapVo.path.length
            for (var i: number = 0; i < $len; i++) {
                var $pos: Vector2D = new Vector2D(GuidData.map.tbMapVo.path[i][0], GuidData.map.tbMapVo.path[i][1]);
            ...
            var $to2D: Vector2D = new Vector2D(GuidData.map.tbMapVo.path[nearId][0], GuidData.map.tbMapVo.path[nearId][1])
            var item: Array<Vector2D> = AstarUtil.findPath2D($self2D, $to2D);
            if (item && item.length) {
                MainCharControlModel.getInstance().setWalkPathFun(item, $fun);
            }
    ...
    public jumpTo(): void {
        ...
        var toPos: Vector3D = m.transformVector(new Vector3D(12 * 10, 0, 0))
