module dialog {
    ...
    export class DialoguePanel extends UIPanel {
        ...
        public static openPanelByTab($entryId: number, $tabId: number): boolean {
            var $entryVo: tb.TB_creature_template = tb.TB_creature_template.get_TB_creature_template($entryId);
            var sdv: tb.TB_creature_dialogue = tb.TB_creature_dialogue.get_TB_creature_dialogue($entryVo.dialogue_id);
            if (sdv.type > 0) {
                if (sdv.type == 1) {
                    if ($tabId == -1) {
        ...
        private showRewaldPanel(): void {
          ...
          //  this._bottomRender.uiAtlas.clearCtxTextureBySkilname(this.a_talk_txt.skinName);
            ...
            for (var i: number = 0; this.selectQuestTaskVo&& i < this.selectQuestTaskVo.tb_quest.rewards[0].length; i++) {
                var $ui: UICompenent = <UICompenent>this.addChild(this._topRender.getComponent("a_rewad_icon_" + i));
