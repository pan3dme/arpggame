module Chat {
    ...
    export class ChatListContextComponent extends UIRenderComponent {
        ...
        private drawSytInfo(): void {
            ...
            var $arr: Array<ChatVo> = ChatModel.getInstance().getChatItemByType(SharedDef.CHAT_TYPE_SYSTEM);

            var $totalNh: number = 0;
            var $beginNum: number = 0
            for (var i: number = ($arr.length - 1); i > 0; i--) {

                var $nh: number = ChatModel.writeMultiFaceLineToCtx(this.uiAtlas.ctx, ColorType.Brown7a2f21 + $arr[i].contentTxt, 23, 100, 0);
                $totalNh += $nh;
                if ($totalNh > 13) {
                    $beginNum = i;
                    i = 0;
            ...
            var th: number = 0
            for (var i: number = $beginNum; i < $arr.length; i++) {
                UiDraw.cxtDrawImg(this.uiAtlas.ctx, PuiData.S_sys_tip1, new Rectangle(10, th * 35 + 40, 63 * 1.2, 30 * 1.2), UIData.publicUi);
                var $contextStr: string = $arr[i].contentTxt.replace("[ffffff]", ColorType.Brown7a2f21)
                var $nh: number = ChatModel.writeMultiFaceLineToCtx(this.uiAtlas.ctx, ColorType.Brown7a2f21 + $contextStr, 23, 100, 43 + th * 35);
                th += $nh
            }
            this.uiAtlas.updateCtx(this.uiAtlas.ctx, 0, 0);
