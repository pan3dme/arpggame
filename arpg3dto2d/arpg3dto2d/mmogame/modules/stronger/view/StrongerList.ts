module stronger {
    ...
    export class StrongerList extends SList {
        ...
        public refreshItem() {
            this._ary = StrongerModel.getInstance().getList(this._type);
            //排序
            if (this._type == StrongerUitl.STRONGER_UP) {
                this._ary.sort(
    ...
    export class StrongerListRender extends SListItem {
        ...
        private drawUnlockBtn() {
            var vo: StrongerItemVo = this.itdata.data
            var $rec: UIRectangle = this._baseRender.uiAtlas.getRec(this.Btn.skinName);
            var ctx: CanvasRenderingContext2D = UIManager.getInstance().getContext2D($rec.pixelWitdh, $rec.pixelHeight, false);
            var imgUseRect1: UIRectangle = this.parentTarget.baseAtlas.getRec("A_unlockbtn")
