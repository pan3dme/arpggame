...
var quaColorAry: Array<string> = [ColorType.Whitefff4d6, ColorType.color2daa35, ColorType.color4392ff, ColorType.colorb759ff, ColorType.colorff7200, ColorType.colorce0a00];
function getColorQua(qua: number): string {
    return this.quaColorAry[qua]
}

var colorNameAry: Array<string> = ["白色", "绿色", "蓝色", "紫色", "橙色", "红色"];
function getQuaName(qua: number): string {
    return this.colorNameAry[qua];
}

var dailyAry: Array<string> = ["日", "一", "二", "三", "四", "五", "六"];
function getDateName(qua: number): string {
...
function getrestaskhardIconUrl(name: string): string {
...
function getStateUpIconUrl(name: string): string {
...
function getTreasureIconUrl(name: string): string {
...
/** 获取npc身上的模块id */
function getNpcmodul(npcid:number): number {
    var $aa: tb.TB_creature_template = tb.TB_creature_template.get_TB_creature_template(npcid)
    var sdv: tb.TB_creature_dialogue = tb.TB_creature_dialogue.get_TB_creature_dialogue($aa.dialogue_id);
...
function costRes($costary: Array<number>,fun:Function,failFun:Function=null):void{
    var curNum:number = GuidData.player.getResType($costary[0]);
    ...
    if($costary[1] <= curNum){
        ...
        if($costary[1] <= curNum){
            AlertUtil.show("绑定元宝不足，将消耗元宝","消耗提示",(flag)=>{
                if(flag == 1){
                    fun();
                }
            });
